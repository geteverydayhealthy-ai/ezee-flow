...
const BlueprintStrategy = () => {
  const { get } = usePageContent("blueprint-strategy");

  const heroHeading = get("hero", "heading", "Blueprint & Strategy Playbook");
  const heroSub = get("hero", "subheading", "A structured discovery and planning engagement that maps your current landscape, defines priorities, and creates a clear digital transformation roadmap before any platform build begins.");
  const whatHeading = get("what_it_is", "heading", "What is the Blueprint & Strategy Playbook?");
  const whatDesc = get("what_it_is", "description", "This is a focused consulting engagement for insurers who are planning digital transformation but need strategic clarity before committing resources. Over 4 to 8 weeks, Ezee Technologies works with your leadership and operations teams to assess your current state, identify operational bottlenecks, define a target architecture, and create a phased implementation roadmap that reduces risk and maximizes impact.");
  const ctaHeading = get("cta", "heading", "Start with the right plan. Build with confidence.");
  const ctaSub = get("cta", "subheading", "Book a Blueprint & Strategy consultation to define your transformation roadmap.");
