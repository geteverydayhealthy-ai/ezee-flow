...
const EmbeddedInsurance = () => {
  const { get } = usePageContent("embedded-insurance");

  const heroHeading = get("hero", "heading", "Embedded Insurance Infrastructure Playbook");
  const heroSub = get("hero", "subheading", "A strategic design engagement that defines how insurance should be distributed through partners, ecosystems, fintechs, digital channels, and white-label environments. With clear architecture, partner roles, and operating models.");
  const whatHeading = get("what_it_is", "heading", "What is the Embedded Insurance Playbook?");
  const whatDesc = get("what_it_is", "description", "Embedded insurance is one of the fastest-growing distribution models in the industry, but executing it well requires more than connecting an API. It requires a clear understanding of partner roles, customer journey design, integration architecture, governance structures, and operating responsibilities. This Playbook helps insurers design their embedded distribution strategy from the ground up, ensuring that every partnership, channel, and integration is structured for scalability, compliance, and commercial success.");
  const ctaHeading = get("cta", "heading", "Design your embedded insurance strategy with clarity");
  const ctaSub = get("cta", "subheading", "Book a consultation to define the right partner, integration, and operating model for embedded distribution.");
    ...
    <Layout>
      ...
      <section className="section-spacing bg-accent/50">
        <div className="section-container max-w-3xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl font-bold mb-6">{whatHeading}</h2>
